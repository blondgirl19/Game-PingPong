package resources;

public interface strings {
    String APP_TITLE = "Настольный теннис";

    //General
    String WARNING = "Предупреждение";
    String ARE_U_SURE_WANT_EXIT = "Вы действительно хотите выйти?";
    String DO_EXIT = "Выйти";
    String STAY = "Остаться";
    String START = "НАЧАТЬ";

    //MAIN MENU
    String MAIN_MENU = "ГЛАВНОЕ МЕНЮ";
    String GAME = "Игра";
    String STATISTICS = "Статистика";
    String SETTINGS = "Настройки";
    String HELP = "Помощь";
    String ABOUT = "Об игре";
    String EXIT = "Выход";

    String ABOUT_GAME_TEXT = "Игра была написана студенткой\n" +
            "ХНУ им. В.Н. Каразина, Анной Сабинской.\n" +
            "В качестве курсовой работы на втором курсе.\n\n" +
            "Сотрудничество:\n" +
            "vk.com/anya_slabinskaya\n" +
            "ask.fm/Annyuta11\n\n" +
            "Спасибо за скачивание!";

    String HELP_TEXT = "* Двигайте левого игрока кнопками \"S, W\".\n" +
            "* Правого - с помощью стрелок на клавиатуре.\n\n" +
            "* Чтобы приостановить игру, нажмите пробел.\n" +
            "* Если мяч и ракетка соприскасаются, двигаясь при этом\n" +
            "в разные стороны, мяч сменит направление движения по Y.\n\n" +
            "* При движении мяча и ракетки в одну сторону \n" +
            "(во время соприкосновения), мяч ускорится\n\n" +
            "* При каждом соприкосновении, если ракетка стоит на месте, мяч\n" +
            "немного ускоряется. Это усложняет игру со временем.\n\n" +
            "* Когда забит гол, ускорение сбрасывается.\n\n" +
            "Пусть победит сильнейший из Вас!";

    //GAME SETTINGS
    String GAME_PARAMS = "Параметры игры";
    String PLAYER_1 = "ИГРОК 1";
    String PLAYER_2 = "ИГРОК 2";
    String NAME = "Имя: ";
    String PLAYER_TYPE = "Тип игрока: ";
    String COMPUTER = "Компьютер";
    String HUMAN = "Человек";
    String HARD = "Сложный";
    String MEDIUM = "Средний";
    String EASY = "Простой";
    String DIFFICULT = "Сложность: ";
    String CHOOSE_COLOR = "Выберите цвет";
    String PLAYER_COLOR = "Цвет игрока: ";
    String ANOTHER_PARAMS = "Другие параметры";
    String BALL_SPEED = "Скорость мяча: ";
    String RACKETS_SPEED = "Скорость ракеток: ";
    String RACKETS_SIZE = "Длинна ракеток: ";
    String BALL_DIAMETER = "Диаметр мяча: ";
    String SCORES_TO_WIN = "Голов до победы: ";
    String GAME_FPS = "FPS: ";
    String DEFAULT = "По умолчанию";
    String DEFAULT_LEFT_PLAYER_NAME = "Anna";
    String DEFAULT_RIGHT_PLAYER_NAME = "Sabinskaya";
    String ERROR ="Ошибка!";
    String NAME_CANNOT_BE_EMPTY = "Имя не может быть пустым";

    //GAME
    String SCORES_FORMAT = "%d : %d";
    String PRESS_SPACE_TO_START = "Нажмите пробел для старта";
    String GAME_END = "Конец игры";
    String PLAYER_WIN_FORMAT = "Игрок \"%s\" победил!";
    String PLAY_AGAIN = "Играть заново";
    String PAUSE = "Пауза";
}
